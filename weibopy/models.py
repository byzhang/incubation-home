...
class Status(Model):
    ...
    def __repr__(self):
        value = {
                'author': self.getAttr(self.getAttr(self, 'author'), 'screen_name').encode('utf-8'),
                'id': self.getAttr(self, 'id'),
                'text': self.getAttr(self, 'text').encode('utf-8'),
                'retweeted': self.getAttr(self, 'retweeted_status')}
        return repr(value)
...
class User(Model):
    ...
    @classmethod
    def parse_dict(cls, api, json_list):
        rtn = {}
        if isinstance(json_list, list):
            item_list = json_list
        else:
